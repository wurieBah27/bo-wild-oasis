...
export const getBookings = async ({ filter, sortBy, page }) => {
  const query = supabase
    .from("Bookings")
    .select(
      "created_at,id,startDate,endDate,numNights,numGuests,status,totalPrice,Cabins(name),guests(fullName,email)",
      { count: "exact" }
    );
  ...
  if (error) {
    ...
    throw new Error(
      "Oh we encountered an error while loading the Bookings. Please try again"
    );
  }
...
export async function updateBooking(id, obj) {
  try {
    ...
    throw new Error("An unexpected error occurred while updating the booking");
  }
}
